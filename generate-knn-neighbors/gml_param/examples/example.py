...
def generate_feature_pickl_only_has_factor(common_name, data_name, data_name_1,TYPE,segment_symbol):
    knn_csv_prefix = r'G:\DNN提取规则\all_result\{}'.format(common_name.split("\\")[-1])
    embedding_csv_prefix = r'E:\服务器上下载的\zcl\transformers-4.3.3\input_data\{}'.format(common_name)
    knn_sim0= pd.read_csv(knn_csv_prefix +  r'\twitter2016_knn_3_of_train_bert-base-uncased_train_train.txt', sep='\t').values
    knn_sim1 = pd.read_csv(knn_csv_prefix +  r'\twitter2016_knn_3_of_test_bert-base-uncased.txt', sep='\t').values
    ...
    for index, binary in enumerate([first_kind]):
        ...
        if index == 0:
            ...
            continue
        feature_id_val = line['feature_id']
        dic_specific = dict()
        for inx, (edge0, edge1, value) in enumerate(binary):
            ...
            edge_tp = (edge0, edge1)
            if edge_tp in dic_specific:
                continue
            dic_specific[edge_tp] = [0.5, value]
        line['weight'] = dic_specific
        feature_id_2_weight[feature_id_val] = dic_specific
        all_lines.append(line)
    # fw = open('./{}/only_one_'.format(data_name_1) + data_name_1 + "_features.pkl", 'wb')
    # pickle.dump(all_lines, fw)

    return var_id_2_feature_ids, feature_id_2_weight, all_lines

def generate_variable_pickl_only_has_factor(data_name_1, data_name, var_id_2_feature_ids, feature_id_2_weight, embedding_csv_prefix):
    ...
    def add_feature_to_variable(vid0, vid1, fid, all_ids_feature):
        if vid0 not in all_ids_feature:
            all_ids_feature[vid0] = dict()
        if featureid not in all_ids_feature[vid0]:
            all_ids_feature[vid0][fid] = list()
        feature_list = all_ids_feature[vid0][fid]
        if (vid0, vid1) not in feature_list and (vid1, vid0) not in feature_list:
            feature_list.append((vid0, vid1))

    all_feature_dict = dict()
    for featureid, feature_w_val in feature_id_2_weight.items():
        for key, value in feature_w_val.items():
            if not isinstance(key, tuple):
                continue
            vidA, vidB = key
            add_feature_to_variable(vidA, vidB, featureid, all_feature_dict)
            add_feature_to_variable(vidB, vidA, featureid, all_feature_dict)
    ...
    for elem in range(len(all_ids)):
        ...
        feature_dic = dict()
        if var_id_val in var_id_2_feature_ids:
            feature_id_list = var_id_2_feature_ids[var_id_val]
            for feature_id in feature_id_list:
                if feature_id in feature_id_2_weight:
                    temp_list = list(feature_id_2_weight[feature_id][var_id_val])
                final_list = [0.5 if temp_list[1] > 0.5 else -0.5, temp_list[1]]
                feature_dic[feature_id] = final_list
        if var_id_val in all_feature_dict:
            features = all_feature_dict[var_id_val]
            for feature_id, feature_list in features.items():
                if feature_id not in feature_dic:
                    feature_dic[feature_id] = feature_list
                else:
                    feature_dic[feature_id].extend(feature_list)
...
def perform_dnn(common_name, segment_symbol):
    ...
    TYPE = data_name
    # os.mkdir("../data/"+'')
    var_id_2_feature_ids, feature_id_2_weight, feature_pkl = generate_feature_pickl_only_has_factor(common_name,data_name,data_name_1,TYPE,segment_symbol)
    var_pkl = generate_variable_pickl_only_has_factor(data_name_1, data_name, var_id_2_feature_ids, feature_id_2_weight, embedding_csv_prefix)
