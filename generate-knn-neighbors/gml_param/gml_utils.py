...
def get_train_ids(variables):
    train_ids = []
    for variable in variables:
        if variable['is_easy'] == True:
            train_ids.append(variable['var_id'])
    return train_ids
    ...
    for var_id in var_id_list:
        ...
        for kv in feature_set.items():
            if features[kv[0]]['parameterize'] == 1:
                if features[kv[0]]['feature_type'] != 'binary_feature' and features[kv[0]]['feature_type'] != 'sliding_window_factor':
                    for interval_index in range(0, len(evidence_interval)):
...
def init_bound(variables,features):
    ...
    '''
    for feature in features:
        if feature['parameterize'] == 1 and feature['feature_type'] != 'binary_feature' and feature['feature_type'] !='sliding_window_factor' and feature['feature_type'] != 'unary_feature':
            ...
            feature['alpha_bound'] = copy([-math.inf,math.inf])
            feature['tau_bound'] = copy([0,10])
        else:
            feature['alpha_bound'] = copy([-math.inf,math.inf])
            feature['tau_bound'] = copy([0,10])
...
def open_p(weight):
    ...
    # return  math.exp(weight) / (1 + math.exp(weight))

# def open_p(self, var_id, weight):
#     '''
#     @param var_id:
#     @param weight:
#     @return:
