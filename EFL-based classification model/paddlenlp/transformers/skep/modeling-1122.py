...
class SkepForSequenceClassification(SkepPretrainedModel):
    ...
    def __init__(self, skep, num_classes=2, dropout=None):
        ...
        self.key = nn.Linear(self.hidden_size, self.all_head_size,bias_attr=True)  # 768*768
        self.value = nn.Linear(self.hidden_size, self.all_head_size,bias_attr=True) # 768*768
        self.unit_linear = nn.Linear(self.unit_dim, num_classes,bias_attr=True) # 64*2
    ...
    def forward(self,
        ...
        my_hidden_states_start = my_hidden_states.unsqueeze(1)
        mixed_query_layer = self.query(my_hidden_states_start)  #4*1*768
        key_layer = self.transpose_for_scores(self.key(my_hidden_states_start))  # 4*1*768  key_layer=4*12*1*64
        value_layer = self.transpose_for_scores(self.value(my_hidden_states_start)) # 4*1*768  value_layer=4*12*1*64
        ...
        unit_all = []
        for i in range(16):
            head_i = self.dense0(context_layer[:,i,:])
            paddle.nn.ReLU()
            head_i = self.LayerNorm(head_i)
            head_i = self.dense0(head_i)
            head_i = self.dropout(head_i)
            head_i = self.LayerNorm(head_i)
            unit_all.append(self.unit_linear(head_i))
        all_cat_unit =None
        for i in range(16):
            str_name = unit_all[i]
            # unit_i = paddle.unsqueeze(str_name, 0)
            if all_cat_unit is None:
                all_cat_unit = str_name
            else:
                all_cat_unit += str_name
        # all_cat_unit = paddle.Tensor.transpose(x=all_cat_unit, perm=[1, 2, 0])
        # preds = self.dense(all_cat_unit)
        # preds = paddle.squeeze(preds,-1)
        preds = all_cat_unit/16
