...
class SkepForSequenceClassification(SkepPretrainedModel):
    ...
    def __init__(self, skep, num_classes=2, dropout=None):
        ...
        self.key = nn.Linear(self.hidden_size, self.all_head_size)  # 768*768
        self.value = nn.Linear(self.hidden_size, self.all_head_size) # 768*768
        self.unit_linear0 = nn.Linear(self.unit_dim, num_classes) # 64*2
        self.unit_linear1 = nn.Linear(self.unit_dim, num_classes) # 64*2
        self.unit_linear2 = nn.Linear(self.unit_dim, num_classes) # 64*2
        self.unit_linear3 = nn.Linear(self.unit_dim, num_classes) # 64*2
        self.unit_linear4 = nn.Linear(self.unit_dim, num_classes) # 64*2
        self.unit_linear5 = nn.Linear(self.unit_dim, num_classes) # 64*2
        self.unit_linear6 = nn.Linear(self.unit_dim, num_classes) # 64*2
        self.unit_linear7 = nn.Linear(self.unit_dim, num_classes) # 64*2
        self.unit_linear8 = nn.Linear(self.unit_dim, num_classes) # 64*2
        self.unit_linear9 = nn.Linear(self.unit_dim, num_classes) # 64*2
        self.unit_linear10 = nn.Linear(self.unit_dim, num_classes) # 64*2
        self.unit_linear11 = nn.Linear(self.unit_dim, num_classes) # 64*2
        self.unit_linear12 = nn.Linear(self.unit_dim, num_classes) # 64*2
        self.unit_linear13 = nn.Linear(self.unit_dim, num_classes) # 64*2
        self.unit_linear14 = nn.Linear(self.unit_dim, num_classes) # 64*2
        self.unit_linear15 = nn.Linear(self.unit_dim, num_classes) # 64*2
        self.out = paddle.nn.Linear(self.attention_head_size, 1024)
        self.dropout = nn.Dropout(self.hidden_dropout_prob)
        self.ffn = paddle.nn.Sequential(
            paddle.nn.Linear(self.attention_head_size, self.attention_head_size),
            paddle.nn.ReLU(),
            paddle.nn.Dropout(0.3),
        )
        # self.pattern_weight = nn.Param(nn.zeros(16))
    ...
    def forward(self,
        ...
        context_layer = self.ffn(context_layer)

        unit_0 = self.unit_linear0(context_layer[:,0])
        unit_1 = self.unit_linear1(context_layer[:,1])
        unit_2 = self.unit_linear2(context_layer[:,2])
        unit_3 = self.unit_linear3(context_layer[:,3])
        unit_4 = self.unit_linear4(context_layer[:,4])
        unit_5 = self.unit_linear5(context_layer[:,5])
        unit_6 = self.unit_linear6(context_layer[:,6])
        unit_7 = self.unit_linear7(context_layer[:,7])
        unit_8 = self.unit_linear8(context_layer[:,8])
        unit_9 = self.unit_linear9(context_layer[:,9])
        unit_10 = self.unit_linear10(context_layer[:,10])
        unit_11 = self.unit_linear11(context_layer[:,11])
        unit_12 = self.unit_linear8(context_layer[:,7])
        unit_13 = self.unit_linear9(context_layer[:,8])
        unit_14 = self.unit_linear10(context_layer[:,9])
        unit_15 = self.unit_linear11(context_layer[:,10])

        # preds = unit_0, unit_1,unit_2,unit_3,unit_4,unit_5,unit_6,unit_7,unit_8,unit_9,unit_10,unit_11)
        ...
        temp = paddle.Tensor.concat([context_layer[:,0], context_layer[:,1],context_layer[:,2],context_layer[:,3],context_layer[:,4],context_layer[:,5],context_layer[:,6],context_layer[:,7],
                                     context_layer[:,8],context_layer[:,9],context_layer[:,10],context_layer[:,11],context_layer[:,12],context_layer[:,13],context_layer[:,14],context_layer[:,15] ], axis=-1)

        context_layer0 = paddle.Tensor.reshape(x=context_layer, shape= [context_layer.shape[0], self.hidden_size]) #  4*1*12*64  4*1*768
        return preds, content, pooled_output_skepmodel, context_layer0
