...
def do_predict(model, tokenizer, data_loader, data_type, data_name, task_label_description):
    ...
    if data_type == 'test' and data_name=='no':
        ...
        np.savetxt(t_pooled_output_skepmodel1_file1, t_pooled_output_skepmodel, fmt="%.17f",  delimiter="\t")
        t_content_file1 = '{}_{}_content_skepmodel_out1.csv'.format(data_type, data_name)
        t_content_file1_pk = open(t_content_file1, 'wb')
        import pickle
        pickle.dump(t_content, t_content_file1_pk)

    all_prediction_probs = np.concatenate(all_prediction_probs, axis=0)

    prediction_pos_probs = np.reshape(all_prediction_probs, (-1, class_num))
