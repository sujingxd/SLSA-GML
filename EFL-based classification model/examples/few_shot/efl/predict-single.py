...
def parse_args():
    ...
    parser.add_argument("--data_name", required=True, type=str, help="The task_name to be evaluated")
...
def do_predict(model, tokenizer, data_loader, data_type, data_name, task_label_description):
    ...
    if data_type == 'test' and data_name=='no':
        ...
        np.savetxt(sentence_out_file0, t_sentence_out, fmt="%.17f",  delimiter="\t")
        sentence_out_file1 = '{}_{}_sentence_out1.csv'.format(data_type, data_name)
        np.savetxt(sentence_out_file1, t_sentence_out, fmt="%.17f",  delimiter="\t")
        # data_type = 'dev'
        # data_name = 'twitter2013'
        # t_pooled_output_skepmodel1_file1 = '{}_{}_output_skepmode1.npy'.format(data_type, data_name)
        # t_pooled_output_skepmodel1_file1_pk = open(t_pooled_output_skepmodel1_file1, 'wb')
        # t_content_file1 = '{}_{}_content_skepmodel_out1.npy'.format(data_type, data_name)
        # t_content_file1_pk = open(t_content_file1, 'wb')
        import pickle
        # np.save(file=t_content_file1_pk, arr=t_content)
        # np.save(file=t_pooled_output_skepmodel1_file1_pk, arr=t_pooled_output_skepmodel1)
        pass

    all_prediction_probs = np.concatenate(all_prediction_probs, axis=0)
    all_prob = all_prediction_probs
    all_prob = paddle.to_tensor(all_prob)
    all_prob = F.softmax(all_prob, 1)
    all_prob_list = list()
    for (all_neg_prob, all_pos_prob) in all_prob:
        all_pos_prob = all_pos_prob.item()
        all_prob_list.append(all_pos_prob)
    ...
    y_preds = y_pred_index
    model.train()
    prediction_pos_probs = paddle.to_tensor(all_prediction_probs)
    ...
    return y_preds, _pos_prob_list, all_prob_list
...
def write_eprstmt(task_name, output_file, pred_labels):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        for idx, example in enumerate(pred_labels):
            test_example["id"] = idx
            test_example["label"] = 1 if pred_labels[idx] == 'Positive' else 0
...
if __name__ == "__main__":
    ...
    test_ds = load_dataset(_read_train,
