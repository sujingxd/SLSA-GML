...
def parse_args(data_name, seed):
    ...
    parser.add_argument(
        ...
        help="The path of checkpoint to be loaded.")
    parser.add_argument(
        "--seed", default=seed, type=int, help="random seed for initialization")
    parser.add_argument(
        '--device',
        choices=['cpu', 'gpu'],
        default="gpu",
...
def do_train():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(train_data_loader, start=1):
            ...
            # loss0 = criterion(all_logits[0],labels)
            # loss1 = criterion(all_logits[1],labels)
            # loss2 = criterion(all_logits[2],labels)
            # loss3 = criterion(all_logits[3],labels)
            # loss4 = criterion(all_logits[4],labels)
            # loss5 = criterion(all_logits[5],labels)
            # loss6 = criterion(all_logits[6],labels)
            # loss7 = criterion(all_logits[7],labels)
            # loss8 = criterion(all_logits[8],labels)
            # loss9 = criterion(all_logits[9],labels)
            # loss10 = criterion(all_logits[10],labels)
            # loss11 = criterion(all_logits[11],labels)
            # loss12 = criterion(all_logits[12],labels)
            # loss13 = criterion(all_logits[13],labels)
            # loss14 = criterion(all_logits[14],labels)
            # loss15 = criterion(all_logits[15],labels)
            # loss += (loss0+loss1+loss2+loss3+loss4+loss5+loss6+loss7+loss8+loss9+loss10+loss11+loss12+loss13+loss14+loss15)
            global_step += 1
    ...
    shutil.rmtree(path=output_dir0)
    return (test_label == y_pred_labels_test).mean(), f1_score
...
if __name__ == "__main__":
    ...
    #         do_train()
    data_names = ['0.25%-mr','0.5%-mr','1%-mr','3%-mr','5%-mr','0.25%-cr','0.5%-cr','1%-cr','3%-cr','5%-cr']
    seeds = [20, 40, 50, 30, 60, 70]
    for data_name in data_names:
        acc_arr = []
        f1_arr = []
        for seed in seeds:
            args= parse_args(data_name, seed)
            acc, f1 = do_train()
            acc_arr.append(acc)
            f1_arr.append(f1)
        # mean_value = sum(acc_arr)/len(acc_arr)
        # var_value = sum([math.pow((val-mean_value),2) for val in acc_arr])/len(acc_arr)
        # standard_var_value = math.sqrt(var_value)
        # results.write('data_name is:'+ data_name+'\t')
        # results.write('model_name is:'+ model+'\t')
        # results.write('standard_var_value is:'+ str(standard_var_value)+'\n')


        mean_value = sum(acc_arr)/len(acc_arr)
        var_value = sum([math.pow((val-mean_value),2) for val in acc_arr])/len(acc_arr)
        standard_acc_value = math.sqrt(var_value)

        mean_value_f1 = sum(f1_arr)/len(f1_arr)
        var_value_f1 = sum([math.pow((val-mean_value_f1),2) for val in f1_arr])/len(f1_arr)
        standard_f1_value = math.sqrt(var_value_f1)

        results.write('data_name is:'+ data_name+'\t')
        # results.write('model_name is:'+ model+'\t')
        results.write('standard_var_value is:'+ str(standard_acc_value)+'\t')
        results.write('standard_f1_value is:'+ str(standard_f1_value)+'\n')
        results.flush()