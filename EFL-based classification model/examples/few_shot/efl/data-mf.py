...
class DataProcessor(object):
    ...
    def get_train_datasets(self, data_name, datasets, task_label_description):
        """See base class."""
        return self._create_examples(data_name, datasets, "train", task_label_description)

    def get_dev_datasets(self, data_name, datasets, task_label_description):
        """See base class."""
        return self._create_examples(data_name, datasets, "dev", task_label_description)

    def get_test_datasets(self, data_name, datasets, task_label_description):
        """See base class."""
        return self._create_examples(data_name, datasets, "test", task_label_description)
...
#     """Processor for the Eprstmt dataset (CLUE version)."""
#
#     def _create_examples(self, datasets, phase, task_label_description):  # pos的在前,neg的在后
#         """Creates examples for the training and dev sets."""
#         # examples = []
...
#         #
#         #             # Todo: handle imbanlanced example, maybe hurt model performance
#         #             if label_text[true_label] == label:
#         #                 new_example["label"] = 1
#         #                 examples.append(new_example)
#         #             else:
#         #                 new_example["label"] = 0
...
#                 elif true_label == '0':
#                     true_label = 'Negative'
#                 for label, label_description in task_label_description.items():  # neg的在前,pos的在后
#                     new_example = dict()
#                     new_example["sentence1"] = example['sentence']
#                     new_example["sentence2"] = label_description
#                     if true_label == label:
...
#                     examples.append(new_example)
#
#             # np.savetxt('test_labels.txt', test_labels ,fmt="%s", delimiter="\t")
#         return MapDataset(examples)

class EprstmtProcessor(DataProcessor):
    """Processor for the Eprstmt dataset (CLUE version)."""

    def _create_examples(self, data_name, datasets, phase, task_label_description):  # pos的在前,neg的在后
        """Creates examples for the training and dev sets."""
        examples = []

        if phase == "train":
            train_data = pd.read_csv('dataset/{}/train.csv'.format(data_name), sep='\t').values
            for each_example in train_data:
                new_example = dict()
                new_example["sentence1"] = each_example[0]
                new_example["sentence2"] = each_example[1]
                new_example["label"] = each_example[2]
                examples.append(new_example)

        elif phase == "dev":
            dev_data = pd.read_csv('dataset/{}/dev.csv'.format(data_name), sep='\t').values
            for each_example in dev_data:
                new_example = dict()
                new_example["sentence1"] = each_example[0]
                new_example["sentence2"] = each_example[1]
                new_example["label"] = each_example[2]
            ...
            test_data = pd.read_csv('dataset/{}/test.csv'.format(data_name), sep='\t').values
