...
class CrossEntropyWithKL(nn.Layer):
    ...
    def forward(self, kl_loss, dec_output, trg_mask, z_mean,z_log_var,label):
    ...
    def __init__(self,
        ...
        self.lstm_cells = nn.LayerList([
            nn.LSTMCell(
                ...
                input_size=embed_dim + 256, hidden_size=256)
...
class VAESeq2SeqModel(nn.Layer):
    ...
    def sampling(self, z_mean, z_log_var):
        ...
        epsilon = paddle.normal(shape=(z_mean.shape[0], 256))
    ...
    def build_distribution(self, enc_final_state=None):
        ...
        enc_hidden = paddle.concat(enc_hidden, axis=-1)  # 24*512
        z_mean_log_var = self.distributed_fc(enc_hidden) # 32*512
        z_mean, z_log_var = paddle.split(z_mean_log_var, 2, -1)
        return z_mean, z_log_var # 32*32,  32*32
    ...
    def forward(self, src, src_length, trg, trg_length):
        # Encoder
        _, enc_final_state = self.encoder(src, src_length)  # 32*256
        ...
        dec_first_hidden, dec_first_cell = paddle.split(
            dec_first_hidden_cell, 2, axis=-1)  # 32*256, 32*256
        ...
        return kl_loss, dec_output, trg_mask,z_mean,z_log_var,latent_z
