...
def create_data_loader(args):
    ...
    if args.dataset == 'yahoo':
        ...
        examples_dev = [
            dev_ds[i]['sentence'].split() for i in range(len(dev_ds))
        ]
        examples_test = [
            test_ds[i]['sentence'].split() for i in range(len(test_ds))
        ]
        # examples = examples.extend(examples_dev)
        examples = examples+examples_dev+examples_test
...
def my_process(name, data_files,splits):
    import pandas as pd
    train_txt = pd.read_csv(data_files[0], sep='\t')['text']
    dev_txt = pd.read_csv(data_files[1], sep='\t')['text']
    test_txt = pd.read_csv(data_files[2], sep='\t')['text']
    return train_txt,dev_txt,test_txt

def my_create_data_loader(args):
    batch_size = args.batch_size
    max_len = args.max_len
    if args.dataset == 'yahoo':
        train_ds, dev_ds, test_ds = load_dataset(
        ...
        #     'ptb', splits=('train', 'valid', 'test'))
        import pandas as pd
        train_file = '/home/ssd1/sj/paddle-yuanshi/PaddleNLP-develop/examples/text_generation/vae-seq2seq/train.csv'
        dev_file = '/home/ssd1/sj/paddle-yuanshi/PaddleNLP-develop/examples/text_generation/vae-seq2seq/dev.csv'
        test_file = '/home/ssd1/sj/paddle-yuanshi/PaddleNLP-develop/examples/text_generation/vae-seq2seq/test.csv'



        train_ds, dev_ds, test_ds = load_dataset(path_or_read_func=my_process,data_files=[train_file,dev_file,test_file],lazy=True)

        # examples = [
        #     train_ds[i]['sentence'].split() for i in range(len(train_ds))
        # ]
        examples = [
            train_ds[i].split() for i in range(len(train_ds))
        ]
    ...
    def convert_example(example):
        features = vocab.to_indices(example.split()[:max_len])
        return features
