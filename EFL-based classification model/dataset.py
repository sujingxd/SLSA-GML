...
def load_dataset(path_or_read_func,
    ...
    """
    if inspect.isfunction(path_or_read_func): # not enter
        ...
        reader_cls = import_main_class(path_or_read_func)  # fewclue
...
class MapDataset(Dataset):
    ...
    def _transform(self, data):
        for fn in self._transform_pipline:#这里_transform_pipline有两个tran_func函数了，第一次传进去的data是对的，是text。第二次传进去的data是第一次处理完后的tuple，然后就错了 我知道了 那怎么修改呢。我看看
            data = fn(data)
        return data
    ...
    def _map(self, fn, lazy=True, batched=False):
        if batched:
            ...
            self._transform_pipline.append(fn)#这里，把trans_func加到_transform_pipline里面了，新加的那段的fn又加到这里了，后面处理数据的时候就调了两次tran_func，其实只要调一次
        else:
            self.new_data = [
    ...
    def read_datasets(self, splits=None, data_files=None):
        ...
        if splits and data_files is None:
            ...
            for split in splits:
                ...
                if split == 'train':
                    filename = '/home/ssd1/sj/paddle-yuanshi/PaddleNLP-develop/examples/text_generation/vae-seq2seq/train.txt'
                elif split == 'test':
                    filename = '/home/ssd1/sj/paddle-yuanshi/PaddleNLP-develop/examples/text_generation/vae-seq2seq/test.txt'
                else:
                    filename = '/home/ssd1/sj/paddle-yuanshi/PaddleNLP-develop/examples/text_generation/vae-seq2seq/dev.txt'
                lock_file = os.path.join(DATA_HOME, self.__class__.__name__)
                if self.name is not None:
                    lock_file = lock_file + "." + self.name
                lock_file += "." + split + ".done" + "." + str(os.getppid())
        ...
        if data_files:
            ...
            if splits:
                ...
                for i in range(len(data_files)):
                    temp = self.read(filename=data_files[i])
                datasets += [
                    # self.read(
