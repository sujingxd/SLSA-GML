...
class Trainer:
    ...
    def compute_loss(self, model, inputs):
        ...
        if self.label_smoother is not None and "labels" in inputs:
            ...
            labels = None
        outputs, sentence_out_batch_ori, sentence_out_batch_srl, sentence_out_batch_dep = model(**inputs)
        # Save past state if it exi_orists
        ...
        return (loss, outputs, sentence_out_batch_ori, sentence_out_batch_srl, sentence_out_batch_dep)
    ...
    def evaluate(
        ...
        return output.metrics, output.predictions, output.sid, output.label_ids
    ...
    def predict(
        ...
        output = self.prediction_loop(
            test_dataloader, description="test", ignore_keys=ignore_keys, metric_key_prefix=metric_key_prefix
        )
        output.metrics.update(speed_metrics(metric_key_prefix, start_time, len(test_dataset)))
        return output

    def train_data_predict(self) -> PredictionOutput:
        train_dataloader = self.get_train_dataloader()
        return self.prediction_loop(train_dataloader, description="train")
    ...
    def prediction_loop(
        ...
        sentence_out: torch.Tensor = None
        sentence_out_srl:torch.Tensor=None
        sentence_out_dep:torch.Tensor=None
        world_size = 1
        if is_torch_tpu_available():
        ...
        for step, inputs in enumerate(dataloader):
            loss, logits, labels, sentence_out_batch, sentence_out_batch_srl, sentence_out_batch_dep = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            if loss is not None:
            ...
            # add by sj
            if ids is None:
                ids = inputs["sid"].detach()
            else:
                ids = torch.cat((ids, inputs["sid"].detach()), dim=0)
            if sentence_out is None:
                sentence_out = sentence_out_batch.detach()
                sentence_out_srl = sentence_out_batch_srl.detach()
                sentence_out_dep = sentence_out_batch_dep.detach()
            else:
                sentence_out = torch.cat((sentence_out, sentence_out_batch.detach()), dim=0)
                sentence_out_srl = torch.cat((sentence_out_srl, sentence_out_batch_srl.detach()), dim=0)
                sentence_out_dep = torch.cat((sentence_out_dep, sentence_out_batch_dep.detach()), dim=0)
        # save matrix
        sentence_out = sentence_out.cpu().numpy()
        # out_all_word_embedding = out_all_word_embedding.cpu().numpy()
        ids = ids.cpu().numpy()
        save_matrix(sentence_out, self.args.output_dir + "{}_sentence_out.tsv".format(description))
        import pickle
        file_out_sentence_out_srl = open(self.args.output_dir + "{}_sentence_srl.pkl".format(description), 'wb')
        pickle.dump(sentence_out_srl, file_out_sentence_out_srl)

        file_out_sentence_out_dep = open(self.args.output_dir + "{}_sentence_dep.pkl".format(description), 'wb')
        pickle.dump(sentence_out_dep, file_out_sentence_out_dep)
        ...
        if self.compute_metrics is not None and preds is not None and label_ids is not None:
            metrics = self.compute_metrics(EvalPrediction(predictions=preds, label_ids=label_ids, sid=ids))
        else:
            metrics = {}
        ...
        return PredictionOutput(predictions=preds, label_ids=label_ids, metrics=metrics, sid=ids)
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if self.use_amp:
                ...
                outputs, sentence_out_batch_ori, sentence_out_batch_srl,sentence_out_batch_dep = model(**inputs)
            if has_labels:
                loss, outputs, sentence_out_batch_ori, sentence_out_batch_srl, sentence_out_batch_dep = self.compute_loss(model, inputs)
        ...
        return (loss, logits, labels, sentence_out_batch_ori,sentence_out_batch_srl,sentence_out_batch_dep)
